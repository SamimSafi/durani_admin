// src/components/ProjectGoalsForm.js
...
const ProjectGoalsForm = () => {
  ...
  const { createProjectGoals, updateProjectGoals, getProjectGoals } = useProjectGoalsStore();
  ...
  // Fetch projectGoals data for editing
  useEffect(() => {
    if (isEdit) {
      const fetchProjectGoalsData = async () => {
        try {
          const projectGoals = await getProjectGoals(id);
          if (projectGoals) {
            setProjectGoalsData(projectGoals);
            setValue('projectId', projectGoals.projectId || '');
            setValue('title', projectGoals.title || '');
            setValue('title_pashto', projectGoals.title_pashto || '');
            setValue('title_dari', projectGoals.title_dari || '');
            setValue('briefSummary', projectGoals.briefSummary || '');
            setValue('briefSummary_pashto', projectGoals.briefSummary_pashto || '');
            setValue('briefSummary_dari', projectGoals.briefSummary_dari || '');
            setValue('isActive', !!projectGoals.isActive);
            const logoFile = await urlToFileObject(projectGoals.image, 'logo');
            setValue('imagefile', logoFile);
          } else {
            toast.error('ProjectGoals record not found.');
            navigate(`/projectGoals/${projectGoals.projectId}`);
          }
        } catch (error) {
          toast.error('Failed to fetch projectGoals data.');
          navigate('/projectGoals');
        }
      };
      fetchProjectGoalsData();
    }
  }, [id, isEdit, getProjectGoals, navigate, setValue]);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      if (isEdit) {
        await updateProjectGoals(id, payload);
      } else {
        await createProjectGoals(payload);
      }
      toast.success(`ProjectGoals ${isEdit ? 'updated' : 'created'} successfully.`);
      navigate(`/projectGoals/${data.projectId}`);
    } catch (error) {
      toast.error(`Failed to ${isEdit ? 'update' : 'create'} projectGoals record.`);
    }
  };
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          <Stack spacing={3}>
            ...
            <Grid container lg={12} md={12}  sm={12} spacing={2} alignItems="center">
              ...
              <Grid item xs={6}>
                <FileUpload
                  ...
                  existingImage={isEdit ? projectGoalsData?.image : null}
                />
              </Grid>
            </Grid>
            ...
            <Box sx={{ display: 'flex', gap: 2, pt: 2 }}>
              ...
              <Button variant="outlined" onClick={() => navigate(`/projectGoals/${isEdit ? projectGoalsData.projectId:projectId}`)} disabled={isSubmitting}>
                Cancel
              </Button>
            </Box>
          </Stack>
